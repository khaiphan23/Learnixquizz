...
export const Navbar: React.FC = () => {
  ...
  // FIX BUG 2: logout() là async — phải await trước khi navigate
  // Trước đây: logout(); navigate('/') → navigate chạy trước khi signOut xong
  // Sau fix: await logout() → Supabase signOut xong → onAuthStateChange fire
  // → user state = null → rồi mới navigate('/login')
  const handleLogout = async () => {
    try {
      await logout();
    } finally {
      // Dùng finally để đảm bảo luôn redirect dù logout có lỗi hay không
    ...
    <nav className="sticky top-0 z-40 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-700">
          ...
          {user ? (
            <>
              <button
                ...
                className="hidden md:flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-xl transition-colors"
              >
              ...
              >
                {user.photoURL
                  ? <img src={user.photoURL} className="h-8 w-8 rounded-full object-cover" alt={user.name} />
                  : <div className="h-8 w-8 rounded-full bg-indigo-600 flex items-center justify-center text-white text-sm font-bold">{user.name[0].toUpperCase()}</div>
                }
              </button>

              {/* FIX: dùng handleLogout thay vì inline arrow function */}
              <button
                onClick={handleLogout}
                className="hidden md:flex items-center gap-1 p-2 text-slate-500 hover:text-red-500 transition-colors"
                title={t.logout}
              >
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 space-y-1">
          ...
          {user ? (
            /* FIX: dùng handleLogout thay vì inline → await đúng */
            <button
              onClick={handleLogout}
            ...
            <>
              <button
                ...
                className="w-full text-left px-3 py-2 rounded-lg text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800"
              >
                {t.login}
              </button>
              <button
                onClick={() => { navigate('/register'); setMenuOpen(false); }}
                className="w-full text-left px-3 py-2 rounded-lg text-sm font-medium text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/20"
              >
                {t.register}
              </button>
            </>
