import React, { useState } from 'react';
  ...
  import { PlusCircle, Sparkles, Trash2, ChevronDown, ChevronUp, Image, Upload, FileText, Clipboard } from
  'lucide-react';
  ...
  export const CreateQuiz: React.FC = () => {
    ...
    // FIX: Dùng finally để đảm bảo setSaving(false) LUÔN được gọi,
    // kể cả khi navigate() thành công hay thất bại.
    // Trước đây: setSaving(false) chỉ nằm trong catch → khi thành công nút mãi loading
    const handleSave = async () => {
      if (saving) return; // Tránh double-submit
      ...
      try {
        ...
        // Chỉ reset saving nếu không navigate được
        // Nếu navigate thành công, component sẽ unmount nên không cần reset
        if (!navigated) {
          setSaving(false);
        }
      }
        ...
        <Card className="p-6 space-y-4 border-emerald-200 dark:border-emerald-800 bg-emerald-50/50 dark:bg-emerald-900/10">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input type="number" min="0" max="20" value={mcCount} onChange={e => setMcCount(Number(e.target.value))}
