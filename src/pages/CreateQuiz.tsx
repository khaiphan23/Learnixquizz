...
export const CreateQuiz: React.FC = () => {
  ...
  const handleSourceChange = (source: 'paste' | 'upload') => {
    ...
    if (source === 'paste') { setUploadedFile(null); setExtractedText(''); }
  };

  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0] || null;
    if (file) { setUploadedFile(file); setExtractedText(''); setGenError(''); }
  };
    ...
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-8 pb-20">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" isLoading={saving} onClick={handleSave} disabled={saving}>{t.save}</Button>
        </div>
      </div>
      ...
      <Card className="p-6 space-y-4 border-emerald-200 dark:border-emerald-800 bg-emerald-50/50 dark:bg-emerald-900/10">
        ...
        <div className="flex gap-2">
          <button onClick={() => handleSourceChange('paste')}
            className={`flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-xl border transition-all ${contentSource === 'paste' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white dark:bg-slate-700 border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300'}`}>
            <Clipboard className="h-4 w-4" />{t.pasteContent}
          </button>
          <button onClick={() => handleSourceChange('upload')}
            className={`flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-xl border transition-all ${contentSource === 'upload' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white dark:bg-slate-700 border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300'}`}>
        ...
        {contentSource === 'paste' && (
          <div className="space-y-2">
            <Textarea value={pastedText} onChange={e => setPastedText(e.target.value)}
              ...
              rows={6} />
            {pastedText && (
              <button onClick={() => setShowPreview(!showPreview)} className="text-sm text-emerald-600 dark:text-emerald-400 hover:underline">
                {showPreview ? 'Ẩn' : 'Xem'} {t.previewExtracted}
              </button>
            )}
            {showPreview && pastedText && (
              <div className="max-h-48 overflow-auto bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-3 text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap">{pastedText}</div>
            )}
          </div>
        )}
        ...
        {contentSource === 'upload' && (
          <div className="space-y-3">
            <div onDrop={(e) => { e.preventDefault(); const file = e.dataTransfer.files[0]; if (file) setUploadedFile(file); }}
            ...
            {extractedText && (
              <>
                ...
                {showPreview && (
                  <div className="max-h-48 overflow-auto bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-3 text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap">{extractedText}</div>
                )}
              </>
            )}
          </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: t.multipleChoice, val: mcCount, set: setMcCount },
            { label: t.trueFalse, val: tfCount, set: setTfCount },
            { label: t.essay, val: essayCount, set: setEssayCount, max: 10 },
          ].map(({ label, val, set, max }) => (
            <div key={label}>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{label}</label>
              <input type="number" min="0" max={max ?? 20} value={val} onChange={e => set(Number(e.target.value))}
        ...
        {genError && <div className="text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded-lg px-3 py-2">{genError}</div>}

        <Button onClick={generateFromContent} isLoading={aiGenerating} className="w-full bg-emerald-600 hover:bg-emerald-700">
          <Sparkles className="h-4 w-4" />{t.generateFromContentBtn}
        </Button>
      </Card>
          ...
          <Card key={q.id} className="p-5 space-y-4">
            ...
            {!collapsed[q.id] && (
              <div className="space-y-3">
                ...
                {q.imageUrl && <img src={q.imageUrl} alt="preview" className="rounded-xl max-h-48 object-cover border border-slate-200 dark:border-slate-600" onError={e => (e.currentTarget.style.display = 'none')} />}
                {q.type !== 'essay' ? (
                  <div className="space-y-2">
                    {q.options.map((opt, oi) => (
                      <div key={oi} className="flex items-center gap-2">
                        <input type="radio" checked={q.correctAnswerIndex === oi} onChange={() => updateQuestion(idx, { correctAnswerIndex: oi })} className="accent-indigo-600 w-4 h-4 flex-shrink-0" />
                        <input value={opt} onChange={e => { const o = [...q.options]; o[oi] = e.target.value; updateQuestion(idx, { options: o }); }}
